...
export default function App() {
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden bg-[#050408] selection:bg-pink-400/30">
      ...
      <header className="fixed top-0 left-0 w-full z-40 p-4 md:p-8 pointer-events-none">
        <div className="max-w-7xl mx-auto flex items-start justify-between">
          <motion.div initial={{ y: -20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="pointer-events-auto">
            <div className="flex items-center gap-3 md:gap-4 mb-2">
              ...
              <div>
                <h1 className="text-lg md:text-2xl font-light tracking-[2px] text-[#FF9EBB] uppercase">全球动物百科</h1>
          ...
          <div className="flex flex-col items-end">
            ...
            <motion.div className="flex flex-nowrap md:flex-wrap gap-2 mt-3 md:mt-4 pointer-events-auto overflow-x-auto no-scrollbar pb-2 max-w-[78vw] md:max-w-none">
              {filterTags.map((tag) => (
                <button
                  key={tag}
                  type="button"
      ...
      <main className="w-full h-full">
        <GlobeView selectedAnimal={selectedAnimal} onSelectAnimal={handleSelectAnimal} animalsData={filteredAnimals} />
      </main>

      <div className="fixed z-50 pointer-events-auto left-4 bottom-[calc(16px+env(safe-area-inset-bottom))] md:left-5 md:bottom-5">
        <button
          type="button"
          onClick={() => setShowEditorNote(true)}
          className="rounded-xl border border-white/20 bg-black/35 backdrop-blur-md px-3.5 py-2 md:px-4 text-[11px] md:text-xs tracking-widest text-white/80 hover:text-white hover:bg-black/50 transition-all"
        >
          编者声明 / Editor Note
        </button>
      </div>
      ...
      <AnimatePresence>
        {showEditorNote && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[70] bg-black/45 backdrop-blur-[2px] flex items-end md:items-center justify-center p-4"
          ...
          >
            <motion.div
              ...
              className="w-full max-w-2xl rounded-2xl border border-white/15 bg-[#0F0C16]/92 shadow-[0_22px_70px_rgba(0,0,0,0.55)] p-6 md:p-7 text-white"
            >
              <div className="flex items-center justify-between gap-4 mb-4">
                <h3 className="text-base md:text-lg tracking-widest text-[#FF9EBB] uppercase">编者声明 / Editor Statement</h3>
                <button
                  type="button"
                  onClick={() => setShowEditorNote(false)}
                  className="rounded-lg border border-white/15 px-3 py-1.5 text-xs text-white/70 hover:text-white hover:bg-white/10 transition-all"
                >
                  关闭 Close
                </button>
              </div>
              ...
              <div className="space-y-4 text-sm leading-relaxed text-white/85">
                <p>
                  本项目受许多类似应用以及朋友们对猫狗与地理探索兴趣的启发而创作。页面中使用的图片主要来自网络公开资源，仅用于学习与展示目的。如有内容涉及侵权，请及时联系，我们将第一时间删除并处理。
                </p>
                <p>
                  This project is inspired by many similar apps and by friends' shared interest in cats, dogs, and geographic exploration.
                  Most images are sourced from publicly available online content for learning and showcase purposes only.
                  If any content infringes on your rights, please contact us and we will remove it immediately.
                </p>
                <p className="text-white/70">
                  制作者 / Creator: <span className="text-white">Keven</span> <br />
                  参与者 / Contributors: <span className="text-white">yaoyao, Pu, and friends</span>
                </p>
              </div>
            </motion.div>
          </motion.div>
